 ...
 *
 */
public class SparseMDP {
	private int numStates;
	private int numActions;
	private int sparseness;
	private ArrayList<int [][]> nextStates;
	private ArrayList<double [][]> transition; //Transition matrix with size numStates by numStates by numActions T_u(s,s')
	 ...
	 */
	public SparseMDP(int s, ArrayList<int[][]> intP, ArrayList<double[][]> p, double [][] r, boolean [] t, int nS, int nA, double df){
		sparseness = s;
		numStates = nS;
		numActions = nA;
		nextStates = intP;
	...
	Matrix sparseTimes(int [][] ns, double [][] t, Matrix v){		
		Matrix v2 = new Matrix(numStates,1,0);
		for(int i = 0; i < numStates; i++){
			double prod = 0;
	...
	private void computeHamiltonian(){
		...
		for (int ia = 0; ia < numActions; ia++){
			hamiltonian.setMatrix(0, numStates-1, ia, ia, 
					reward.getMatrix(0, numStates - 1, ia, ia).plus(sparseTimes(nextStates.get(ia), transition.get(ia), value.times(gamma))));			
		}
		for (int ia = 0; ia < numActions; ia++)
	...
	private Matrix TransitionGivenPolicy(){
		...
		for (int i = 0; i < numStates; i++){
			for(int j = 0; j < sparseness; j++){
				PPi.set(i, nextStates.get(getPolicy(i))[i][j], transition.get(getPolicy(i))[i][j]);
			}
		}
		return PPi;
	}
	...
	public void SARSA(double lambda, int maxIter, double epislon){
		...
		while(iter < maxIter * numActions * numStates){
			...
			do{//Repeat for each episode	
				t++;
				int s2 = mc.nextState(s,numStates, getTransitionMatrix(a));
	...
	double [][] getTransitionMatrix(int a){
		double [][] t = new double[numStates][numStates];
		for(int i = 0; i< numStates; i++){
			for(int j = 0; j < sparseness; j++){
				t[i][nextStates.get(a)[i][j]] = transition.get(a)[i][j];
			}
		}
		return t;
