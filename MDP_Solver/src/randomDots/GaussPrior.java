...
public class GaussPrior {
	...
	static double[][] buildReward() {
		...
		for (int i = 0; i < numStates; i++) {
			...
			L[i][1] = (prob_P * R_P * prior + (1 - prob_P) * R_N * (1 - prior)) 
			          / (prior * prob_P + (1- prior) * (1 - prob_P));
			L[i][2] = (prob_P * R_N * prior + (1 - prob_P) * R_P * (1 - prior)) 
					  / (prior * prob_P + (1- prior) * (1 - prob_P));
		}
	...
	static void sparseTran(double[][] nextP, int[][] nextState, int s) {
		...
		int t = muTime[1];
		double mu =computeMu(muTime[0],t);
		NormalDist n0 = new NormalDist(mu, sigma * Math.sqrt(1.0/t));

		NormalDist n = new NormalDist(mu, sigma * Math.sqrt(1.0 + 1.0/t));
		double sumP = 0;
		for (int i = 0; i < sparseness; i++) {
			//Given current state (u, t), posterior state is (u + i, t + 1)
			// for i = 0, sparseness - 1
			double o = computeMu(i,1);
			NormalDist n2 = new NormalDist(computeMu(muTime[0]+i,t+1), 
					sigma * Math.sqrt( 1.0/(t+1) ));

			nextP[s][i] = n.density(o) *
					(prior + (1 - 2 * prior) * n2.cdf(0))
					/ (prior + (1 - 2 * prior) * n0.cdf(0));
			sumP += nextP[s][i];
	...
	static void computeBoundary(int[] Policy, String filename) {
		PrintStream fstream;
		try {
			fstream = new PrintStream("Matlab/Policy/GaussPolicy_" + filename + ".txt");
	...
	static public void main(String[] args) {
		...
		String rewardLable = String.format("%.2f_%.1f_%.1f_%.1f_%d", prior, R_S, R_P, R_N, numSamples-1);
		computeBoundary(Policy, rewardLable);
		System.out.println(rewardLable + " Done");
	}
