...
public class BetaPrior {
	...
	static double R_P = 50;
	static double R_N = 0;
	static double prior = 0.8;
	static double alpha_init = 1;
	static double beta_init = 1;
	static int sparseness = 2; // projection onto coherent direction
	...
	static MDP buildMDP(){
		...
		for(int i = 0; i < numSamples - 1; i++){ 
	        ...
	        //Probability that the next belief is b2R(next observation is O_R)?
				BetaDist b = new BetaDist(j + alpha_init, 
						i +  beta_init);
				BetaDist b2 = new BetaDist(j + alpha_init + 1, 
						i +  beta_init);
				
				double P_b2R = ( j + alpha_init ) / (i + j  + alpha_init + beta_init)
						* (prior  * (1 - b2.cdf(0.5)) + ( 1 - prior) * b2.cdf(0.5))
						/ (prior * (1 - b.cdf(0.5)) + (1 - prior) * b.cdf(0.5) );
	...
	static public void main(String [] args) throws IOException, InterruptedException{
		...
		Utilities.computeBoundary(Policy, "biased");
		Utilities.printPolicy(Policy, "biased");
		Utilities.printValue(m.getValue(), "biased");
